"""
EMA 9/21 Crossover strategy with VWAP and RSI confirmation.
...
class EMACrossoverStrategy(BaseStrategy):
    ...
    def generate_signal(self) -> str:
        df = self.df
        if len(df) < max(settings.EMA_SLOW, 20) + 2:
            return "HOLD"
        ...
        rsi = curr.get("rsi", 50)
        vol_ok = curr["volume"] > 1.5 * curr.get("vol_avg", 0) if curr.get("vol_avg", 0) > 0 else True
        above_vwap = curr["close"] > curr.get("vwap", curr["close"])
        below_vwap = curr["close"] < curr.get("vwap", curr["close"])
